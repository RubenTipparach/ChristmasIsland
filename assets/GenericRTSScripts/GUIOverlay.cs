...
public class GUIOverlay : MonoBehaviour {
	...
	/// <summary>
	/// Drawing a GUI, this will need more work/modules.
	/// </summary>
	void OnGUI()
	{
	...
	private void MouseSelectProc()
	{
		...
		if (Input.GetMouseButton(0) && InSelectState)
		{
			...
			foreach (Object o in gb)
			{
				...
				if (obj.GetComponent<Renderer>().isVisible && selection.Contains(screenPos)
	...
	/// <summary>
	/// Movement and Unit Action system.
	/// </summary>
	private void HandleUnitActions()
	{
		...
		foreach (SpaceObject o in gb)
		{
			...
			if (Input.GetMouseButton (0) && Input.GetKey (KeyCode.LeftShift))
			{
				...
				{
					//might have to extend some kind of class or inerface to generalize all units
					var unitWorker = o.transform.GetComponent<Worker>();
					var unitSwordsman = o.transform.GetComponent<Swordsman>();

					if (unitWorker != null)
					{
						SetWorkerTask(o, unitWorker, ray, hit, unitsCount, i);
						textTesting.text = "Worker selected!";
					}
					else if (unitSwordsman != null)
					{
						SetSwordsmanTask(o, unitSwordsman, ray, hit, unitsCount, i);
						textTesting.text = "Swordsman selected!";
					}
					else
					{
						SetSimpleTask(o, ray, hit, unitsCount, i);
					}
				}
			}
		...
		foreach (SpaceObject o in gb)
		{
			if (o != null)
			{
					...
					&& Screen.height - screenPos.y < Screen.height)
				{
					...
					int ipos = o.transform.GetComponent<SimpleCharacterMove>().GetFormationPos;

					GUI.Label(new Rect(screenPos.x, Screen.height - screenPos.y, o.ObjName.Length * 10, 20), o.ObjName + ipos);
				}
			}
		}
	}
	...
	/// <summary>
	/// The display and draw methods should be deprecated.
	/// </summary>
